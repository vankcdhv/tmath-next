        ...
        <div className="lecture pt24 pb24 pos-relative primary-content d-flex d-flex-wrap">
          <div className="w-100">
            ...
            <div className="h-100 d-flex mt24 d-flex-wrap">
              {lectures.map(item => <ImageCard customeClass={'mr12'} key={item.id} title={item.title} url={item.url || '/post/' + item.type + '/' + item.id} content={item.content} image={item.image[0]} />)}
            </div>
          </div>
        </div> : <div/>
      }
